...
type Origin = 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left' | 'bottom-center';
...
void main() {
  ...
  vec2 rayPos;
  if (iIsCenter > 0.5) {
    rayPos = vec2(iResolution.x * 0.5, -0.2 * iResolution.y);
  } else {
    rayPos = vec2(iResolution.x * 1.1, -0.5 * iResolution.y);
  }
  ...
  gl_FragColor = color;
      ...
      const uniforms = {
        ...
        iIsCenter: { value: origin === 'bottom-center' ? 1.0 : 0.0 }
      };
  ...
  useEffect(() => {
    ...
    u.iIsCenter.value = origin === 'bottom-center' ? 1.0 : 0.0;
